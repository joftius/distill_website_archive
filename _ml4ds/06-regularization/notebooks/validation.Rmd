...
```{r}
# x_variables <- select predictors
# X <- model.matrix(~. -1, data = x_variables) 
# Y <- ...
```

If your dataset has a binary outcome, use penalized logistic regression instead of penalized linear regression. See the `family = "binomial"` option in the `glmnet` function below (ignore this comment if your data has a numeric outcome)
...
```{r, eval = FALSE}
# remove eval = FALSE from above
n <- length(Y) # nrow(X)
train <- sample(1:n, floor(4*n/5), replace = FALSE) # some ratio
...
Question: is `replace = FALSE` important? Why or why not?
...
```{r}
# ridge_fit <- glmnet(..., alpha = 0)

```


#### Choose some arbitrary value of `lambda` and check the `coef()` of the resulting models, compare the coefficient estimates between lasso and ridge (check `?coef.glmnet`)

```{r}
# lam <- 42
# coef(ridge_fit, s = lam)
```
...
```{r}
# predict(ridge_fit, s = lam, newx = X_train, etc)
```
